...
CREATE
OR REPLACE FUNCTION uf_datetime2string(dt_ IN DATE)
    RETURN VARCHAR2
IS
    ti NUMBER;
    ret_
...
BEGIN
    ...
    ti
:=  to_date(SYSDATE,'YYYY-MM-DD HH24:MI') - to_date(dt_,'YYYY-MM-DD HH24:MI');

    IF
ti < 1 THEN ret_:='방금';
    ELSIF
ti < 60 THEN ret_:= CONCAT(TRUNC(ti ,0), '분전');
    ELSIF
ti < 60*24 THEN
...
END IF;
    ELSIF
ti < 60*24*7 THEN ret_:= CONCAT(TRUNC(ti/60/24 ,0), '일전');
    ELSIF
ti < 60*24*7*4 THEN ret_:= CONCAT(TRUNC(ti/60/24/7 ,0), '주전');
    ELSIF
(MONTHS_BETWEEN(dt_, SYSDATE)=1) THEN ret_:= '지난달';
    ELSIF
(MONTHS_BETWEEN(dt_, SYSDATE)<13) THEN
...
CREATE
OR REPLACE FUNCTION makeCalendar(startdate IN VARCHAR2, enddate IN VARCHAR2)
    RETURN VARCHAR2
IS
...
varchar2(10);
    sdate
date := to_date(startdate,'YYYY-MM-DD');
    edate
...
CREATE
OR REPLACE FUNCTION getColor4Alert(TSSTARTDATE_ IN VARCHAR2 , TSENDDATE_ IN VARCHAR2 , TSENDREAL_ IN VARCHAR2 , TSRATE_ IN NUMBER)
RETURN VARCHAR2
IS
...
TSRATE_ < 100 THEN
        IF TSSTARTDATE_ > TODAY_ THEN
            BGCOLOR_ := 'gray';
        ELSIF
TSENDDATE_ < TODAY_ THEN
            BGCOLOR_ := 'red';
        ELSIF
TO_DATE(TSSTARTDATE_, 'YYYY-MM-DD') - TRUNC(SYSDATE) < TRUNC(SYSDATE) - TO_DATE(TSENDDATE_, 'YYYY-MM-DD') THEN
            BGCOLOR_ := 'green';
        ELSIF
TO_DATE(TSSTARTDATE_, 'YYYY-MM-DD') - TRUNC(SYSDATE) >= TRUNC(SYSDATE) - TO_DATE(TSENDDATE_, 'YYYY-MM-DD') THEN
            BGCOLOR_ := 'yellow';
